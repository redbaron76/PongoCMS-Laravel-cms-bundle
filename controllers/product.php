...
class Cms_Product_Controller extends Cms_Base_Controller {
	...
	public function get_index($lang = LANG)
	{
		...
		Asset::container('footer')->add('product', 'bundles/cms/js/sections/product_list.js', 'cms');

		//SET ACTIVE LANG
		Session::put('LANG', $lang);

		$this->layout->header_data = array(
			'title' => LL('cms::title.products', CMSLANG)
		);

		$this->layout->top_data = array(
			'search' => '/cms/product/search',
			'q' => ''
		);

		//GET PRODUCT DATA
		$data = CmsProducts::with(array('user'))
				->where_lang($lang)
				->order_by('updated_at', 'desc')
				->order_by('created_at', 'desc')
				->paginate(Config::get('cms::settings.pag'));

		$this->layout->content = View::make('cms::interface.pages.product_list')
		->with('data', $data)
		->with('lang', $lang);
